...
4. Author decisions queued: estimator initial-state priors; AI-intensity process; **naive benchmark for long-horizon trajectories** (candidates: driftless per-element random walk in log relative capability, RW with estimated drift, hold-2025-constant; route: candidates + grounds -> Q&A -> registered rule -> prospective registration for future evaluation years; then S9 question 3 cites the registration -- decision recorded in `papers/paper-a-measurement/REVIEWS.md` Disposition, A5). Paper A section drafts need author review (drafted by continuation session; cross-AI review applied A1-A4, see REVIEWS.md).

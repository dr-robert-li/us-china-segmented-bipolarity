...
1. **The ceiling prior was not the one committed.** The first implementation drew the saturation ceilings from a uniform surrogate rather than the committed lognormal form -- a truncation that understated the attainable saturation drag -- and was corrected against the priors file.
...
3. **A wider set of sampler-transcription defects that produced a quantitatively specific, entirely false diagnosis.** The audit that followed found the first sampler had substituted surrogates for four committed distributions in all. Run 002's diagnosis of its own gate failure -- a plausible, well-formed, quantitative story about a structural saturation limitation of the model -- was an artefact of one of those surrogates: acting on it would have widened a pre-registered saturation prior in exactly the direction that made the failing gate pass. The first run whose sampler matched the priors file showed the diagnosed limitation did not exist.
...
- **PP3.** The growth-innovation stream received its own scale **by dated amendment to the priors file**. The original commitment gave the level and growth innovations one shared scale; the amendment supersedes that line prospectively, and was selected from the candidate list committed before selection on a ground stated without reference to any gate outcome (the declined alternative would have made the gate unfailable by construction). The new scale was itself derived by a procedure committed before its value was computed: the pooled AR(1) residual scale of observed log capacity growth for both states over 2002--2025, from a content-addressed snapshot of the source series. The derivation procedure's commit precedes the value's computation in the history, which is checkable. This remedy is a genuine change to a registered prior, made after a gate failure, and the paper's claim about it is confined to what the record supports: the change is dated, grounded independently of the gate, and readable against the committed failing configuration.
